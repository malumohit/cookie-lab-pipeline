# runner_safari_manual.py — Safari runner (manual)
# Normal Safari window. Assumes extension is already installed & enabled via App Store.
# Fresh session; redirect/refresh/new-tab watch; cookie diffs; writes to Excel.
...
# ===== TARGET COOKIES (same as Chromium runner) =====
TARGET_ORDER = [
...
_CANON_EXACT = {n.lower(): n for n in TARGET_ORDER if not n.endswith("*") and not n.endswith("_")}
_PREFIXES = []
for n in TARGET_ORDER:
    if n.endswith("*"):
        _PREFIXES.append(n[:-1].lower())
    elif n.endswith("_"):
        _PREFIXES.append(n.lower())
...
def _get_nav_marker(driver):
    try:
        # Safari supports performance.timeOrigin in recent versions
    ...
    while (time.time() - t0) < window_sec:
        ...
        for h in list(handles - seen_handles):
            try:
                ...
                seen_handles.add(h)
        try:
            # switch back to original if exists
...
def run_one(job: dict, src_workbook: Path, out_workbook: Path):
    """
    job keys expected (same style as Chromium):
      - browser: "safari" (optional, for logging)
      - affiliate_link: str
      - redirect_window_sec: float (optional)
      - extension_name / extension_version / privacy_name / job_id (optional)
    """
    opts = SafariOptions()
    # No headless; no per-run extension side-load in Safari
    driver = webdriver.Safari(options=opts)
    browser_ver = (driver.capabilities or {}).get("browserVersion") or ""
    ...
    try:
        ...
        print("\n=== MANUAL NAVIGATION (Safari) ===")
        print("The browser is open at the link. Extension must already be enabled in Safari.")
        ...
        while True:
            ...
            if ans in ("y", "yes"):
                ...
                while True:
                    ...
                    elif q in ("n","no"):  popup_seen = "No";  break
...
def _write_rows(job, src_workbook, out_workbook, driver, browser_ver, domain,
    ...
    wide = {
        ...
        "Browser": job.get("browser","Safari"),
    ...
    print("✔ Wrote: Clean_Data + Diagnostics + Cookie Field Comparison (Safari).")
