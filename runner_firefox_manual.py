...
# Observe redirect/refresh + tabs for a short window right after pressing the extension button
def _observe_redirect_refresh_and_tabs(driver, pre_url, pre_nav_ts, pre_handles, window_sec=6.0):
    ...
    while (time.time() - t0) < window_sec:
        ...
        try:
            # try to switch back to (first seen) original if possible
...
def run_one(job: dict, src_workbook: Path, out_workbook: Path):
    ...
    Manual flow:
      ...
      4) Click the extension popup; press ENTER — we sample redirect/refresh, take 'after coupon' snapshot, and log new tabs.
    """
    ...
    try:
        ...
        # Defaults so we never reference unassigned locals
        redirect_url_final = ""
        refreshed = False
        new_tabs = []
...
def goto_comparison_and_write(job, src_workbook, out_workbook,
    ...
    # Add all NON-TARGET cookies that changed (added/removed/value-changed)
